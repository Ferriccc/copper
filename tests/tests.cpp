...
#define ASSERT(x)                                                              \
  if (!(x)) {                                                                  \
    std::cerr << redColorCode << "CHECK FAILED: " << resetColorCode << #x      \
              << std::endl;                                                    \
    exit(1);                                                                   \
  }

#define SUCCESS(x)                                                             \
  std::cerr << greenColorCode << "WORKS: " << resetColorCode << x << std::endl;

#define DBG(x)                                                                 \
  std::cerr << yellowColorCode << "DBG: " << resetColorCode << x << std::endl;
...
void compile() {
  int ret = system("(cd /home/shrey/copper/ && make clean && make test)");
...
void verifyFiles(const toml::table &tbl) {
  if (const toml::array *arr = tbl["exp_files"].as_array()) {
    arr->for_each([&](const auto &o) { ASSERT(fs::exists(o.value_or(""))); });
  }
...
void tester(std::string &path) {
  ...
  if (const toml::array *arr = tbl["make_symlinks"].as_array()) {
    arr->for_each([&](const auto &ele) {
      const toml::table t = *ele.as_table();

      std::string from = *t["from"].value<std::string>(),
                  to = *t["to"].value<std::string>();

      fs::create_symlink(from, to);

      ASSERT(fs::is_symlink(to));
    });
  }

  if (const toml::array *arr = tbl["make_files"].as_array()) {
    arr->for_each([&](const auto &ele) {
      const toml::table t = *ele.as_table();
